...
- Stage 3 must keep skeleton creation and craft polish as separate task boundaries. For user-facing factory apps, the planned `polish` phase must include at least one task with `specialtyRole: "design-engineer"` after foundation/core surfaces exist, so implementation-runner explicitly loads the design-engineer role for motion, component states, touch feel, layout fit, and screenshot readiness.
