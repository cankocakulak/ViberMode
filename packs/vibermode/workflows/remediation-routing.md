...
> Canonical failure-routing workflow for turning validation and review findings into updated task state.

## Pipeline

```text
validation-report.md and/or review.md → remediation-router → updated tasks/run-state
```

Use this workflow after `spec-to-code` reports validation failure, review failure, or a runnable blocker that should be reflected in task state.
...
Purpose:
Apply validator/reviewer routing decisions to `tasks.json` and `run-state.json` so the next implementation run starts from a clean state.
...
- `spec-to-code` should produce validator/reviewer findings; `remediation-routing` should mutate task state.
